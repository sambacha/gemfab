...
contract Gem {
  ...
  bytes32 immutable DOMAIN_SUBHASH =
  ...
  function ward(address usr, bool authed) external payable {
    if (!wards[msg.sender]) revert ErrWard();
    wards[usr] = authed;
    emit Ward(msg.sender, usr, authed);
  }

  function mint(address usr, uint256 wad) external payable {
    if (!wards[msg.sender]) revert ErrWard();
  ...
  function burn(address usr, uint256 wad) external payable {
    if (!wards[msg.sender]) revert ErrWard();
  ...
  function transfer(address dst, uint256 wad) external payable returns (bool ok) {
    unchecked {
      ok = true;
  ...
  function permit(
    ...
    if (signer == address(0)) {
      revert ErrPermitSignature();
    }
    if (owner != signer) {
      revert ErrPermitSignature();
    }
    if (block.timestamp > deadline) {
      revert ErrPermitDeadline();
    }
  }
}
...
contract GemFab {
  ...
  function build(string memory name, string memory symbol) external payable returns (Gem gem) {
